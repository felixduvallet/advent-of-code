...
from day1 import to_list


def to_nested_list(numbers):
    split = numbers.split('\n')
    matrix = []
    for s in split:
        str_numbers = s.split(' ')
...
def checksum(nested_list):
    return sum([max(x) - min(x) for x in nested_list])


def checksum_divisible(nested_list):
    s = 0
    for numbers in nested_list:
        for items in itertools.combinations(numbers, 2):
            sorted = (max(items), min(items))  # Make sure we divide large by small.
            d = divmod(*sorted)
            # If the remainder is zero, add the quotient.
            if d[1] == 0:
                s += d[0]
                continue
    return s
...
data = """4347	3350	196	162	233	4932	4419	3485	4509	4287	4433	4033	207	3682	2193	4223
...
data_div="""4347	3350	196	162	233	4932	4419	3485	4509	4287	4433	4033	207	3682	2193	4223
648	94	778	957	1634	2885	1964	2929	2754	89	972	112	80	2819	543	2820
400	133	1010	918	1154	1008	126	150	1118	117	148	463	141	940	1101	89
596	527	224	382	511	565	284	121	643	139	625	335	657	134	125	152
2069	1183	233	213	2192	193	2222	2130	2073	2262	1969	2159	2149	410	181	1924
...
class TestCheckSum(unittest.TestCase):
    def test_read(self):
        matrix = to_nested_list(example_data)
    ...
    def test_true(self):
        parsed = data.replace('\t', ' ')
        matrix = to_nested_list(parsed)
        val = checksum(matrix)
        self.assertEqual(47136, val)

    def test_divisible_example(self):
        matrix = to_nested_list(example_div)
        val = checksum_divisible(matrix)
        self.assertEqual(9, val)

    def test_divisible(self):
        parsed = data_div.replace('\t', ' ')
        matrix = to_nested_list(parsed)
        val = checksum_divisible(matrix)
        self.assertEqual(250, val)
